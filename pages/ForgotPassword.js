...
export default function ForgotPassword({ navigation }) {
  ...
  const fetchUserFromEmail = async (emailV) => {
    var obj = { email: emailV };
    var js = JSON.stringify(obj);
    // console.log(js);
    try {
      const response = await fetch(bp.buildPath("api/searchUsers"), {
        method: "POST",
        body: js,
      ...
      if (data.id <= 0) {
        setValidState(true);
        setErrorMessage("Email Not Registered to an account");
        console.log("Email Not Registered to an account");
      } else {
        console.log("Navigate to email verification code page");
        navigation.navigate("PasswordCode", { email: data.un });
      }
    } catch (error) {
